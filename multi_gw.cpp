...
void Z_def_new(int i, double p, int n, int k, NumericMatrix::Row Z){
  // do something
  int def_m = i + n;
  int def = R::rbinom(def_m, p);
  def_m -= def;
...
// runs the whole simulation
//[[Rcpp::export]]
NumericMatrix Z_mat_new(double p, int n, int k, int hours, NumericVector start) {
  NumericMatrix Z_m (hours+1, k+1);
  Z_m(0, _) = start;
  for(int gen = 1; gen <= hours; gen++) {
    for(int i = 0; i <= k; i++) {
      for(int j = 0; j < Z_m(gen-1, i); j++) {
        Z_def_new(i, p, n, k, Z_m(gen,_));
      }
    }
  }
  return(Z_m);
...
//[[Rcpp::export]]
NumericMatrix M_est_new(double p, int n, int k) {
  NumericMatrix M (k+1, k+1);
  for(int i = 0; i <= k; i++) {
    Z_def_new(i, p, n, k, M(i,_));
  }
  return(M);
}
